...
public class LMSTintDemo extends ApplicationAdapter {
...
//    public static final int SCREEN_HEIGHT = 862;
    ...
    private static final float GRAY = ColorTools.fromRGBA(0.5f, 0.5f, 0.5f, 1f);
    private float sigL = ColorTools.signalL(GRAY), sigM = ColorTools.signalM(GRAY), sigS = ColorTools.signalS(GRAY),
            opacity = 1f;
...
//        config.setResizable(false);
        ...
        final LMSTintDemo app = new LMSTintDemo();
    ...
    @Override
    public void create() {
        System.out.printf("%1.6f, %1.6f, %1.6f\n", sigL, sigM, sigS);
        ...
        String fragmentShader =
                "#ifdef GL_ES\n" +
                        ...
                        "    vec3 lms = pow(mat3(0.313921f, 0.151693f, 0.017753f, 0.639468f, 0.748209f, 0.109468f, 0.0465970f, 0.1000044f, 0.8729690f) \n" +
                        "        * tgt.rgb, forward)\n" +
                        "        + v_color.rgb - 0.7451;\n" +
                        "    lms = mat3(5.432622f, -1.10517f, 0.028104f, -4.67910f, 2.311198f, -0.19466f, 0.246257f, -0.20588f, 1.166325f) * pow(lms, reverse);\n" +
                        "    gl_FragColor = vec4(clamp(lms, 0.0, 1.0), v_color.a * tgt.a);\n" +
...
//        load("samples/Among_the_Sierra_Nevada_by_Albert_Bierstadt.jpg");
    ...
    public void render() {
        ...
        if (screenTexture != null) {
            ...
            batch.setColor(sigL, sigM, sigS, opacity);
            batch.begin();
    ...
    public void handleInput() {
        if (input.isKeyPressed(Input.Keys.V)) // view
            System.out.println("I=" + sigL + ",P=" + sigM + ",T=" + sigS);
        else if (input.isKeyPressed(Input.Keys.P)) // portrait
            load("samples/Mona_Lisa.jpg");
            ...
            if (input.isKeyPressed(Input.Keys.L)) //light
                sigL = MathUtils.clamp(sigL + (UIUtils.shift() ? -0x3p-7f : 0x3p-7f), 0f, 1f);
            else if (input.isKeyPressed(Input.Keys.M)) //warm
                sigM = MathUtils.clamp(sigM + (UIUtils.shift() ? -0x3p-7f : 0x3p-7f), 0f, 1f);
            else if (input.isKeyPressed(Input.Keys.S)) //mild
                sigS = MathUtils.clamp(sigS + (UIUtils.shift() ? -0x3p-7f : 0x3p-7f), 0f, 1f);
