...
/**
 * Contains code for manipulating colors as {@code int} and packed {@code float} values in the RGB color space.
 ...
 * {@link com.github.tommyettinger.colorful.pure.ipt.ColorTools#fromRGBA(float, float, float, float)} and
 * {@link com.github.tommyettinger.colorful.pure.ycwcm.ColorTools#toRGBA(float)}).
 */
public class ColorTools {
	/**
	 * Gets a packed float representation of a color given as 4 float components, here, R (red), G (green), B (blue),
	 * and A (alpha or opacity). All channels range
	...
	public static float rgb(float red, float green, float blue, float alpha) {
		return BitConversion.intBitsToFloat(((int) (alpha * 255) << 24 & 0xFE000000) | ((int) (blue * 255) << 16 & 0xFF0000)
	...
	public static int toRGBA8888(final float packed) {
		final int rev = Integer.reverseBytes(BitConversion.floatToRawIntBits(packed));
		return rev | (rev >>> 7 & 1);
	}
	...
	public static float fromRGBA8888(final int rgba) {
		return BitConversion.intBitsToFloat(Integer.reverseBytes(rgba) & 0xFEFFFFFF);
	}
	...
	public static float floatGetHSL(float hue, float saturation, float lightness, float opacity) {
		if (lightness <= 0.001f) {
			...
			return BitConversion.intBitsToFloat((((int) (opacity * 255f) << 24) & 0xFE000000) | 0x00FFFFFF);
		} else {
...
//		return (x - li); // (Math.min(li, 1f - li) + 1e-10f);
	...
	public static float lighten(final float start, final float change) {
		final int s = BitConversion.floatToRawIntBits(start), r = s & 0xFF, g = s >>> 8 & 0xFF, b = s >>> 16 & 0xFF,
				a = s & 0xFE000000;
		return BitConversion.intBitsToFloat(
	...
	public static float raiseR(final float start, final float change) {
		final int s = BitConversion.floatToRawIntBits(start), t = s & 0xFF, other = s & 0xFEFFFF00;
		return BitConversion.intBitsToFloat(((int) (t + (0xFF - t) * change) & 0xFF) | other);
	}
	...
	public static float lowerR(final float start, final float change) {
		final int s = BitConversion.floatToRawIntBits(start), t = s & 0xFF, other = s & 0xFEFFFF00;
		return BitConversion.intBitsToFloat(((int) (t * (1f - change)) & 0xFF) | other);
	}
	...
	public static float raiseG(final float start, final float change) {
		final int s = BitConversion.floatToRawIntBits(start), t = s >>> 8 & 0xFF, other = s & 0xFEFF00FF;
		return BitConversion.intBitsToFloat(((int) (t + (0xFF - t) * change) << 8 & 0xFF00) | other);
	}
	...
	public static float lowerG(final float start, final float change) {
		final int s = BitConversion.floatToRawIntBits(start), t = s >>> 8 & 0xFF, other = s & 0xFEFF00FF;
		return BitConversion.intBitsToFloat(((int) (t * (1f - change)) & 0xFF) << 8 | other);
	}
	...
	public static float editRGB(float encoded, float addR, float addG, float addB, float addAlpha,
		...
		final int s = BitConversion.floatToRawIntBits(encoded), r = s & 0xFF, g = s >>> 8 & 0xFF, b = s >>> 16 & 0xFF,
				a = s >>> 25;
		return BitConversion.intBitsToFloat(
	...
	public static float randomEdit(final float color, long seed, final float variance) {
		...
		for (int j = 0; j < 50; j++) {
			...
			z += b;
			if(dist <= limit)
				return BitConversion.intBitsToFloat((decoded & 0xFE000000) | ((int)(z * 255.999f) << 16 & 0xFF0000)
