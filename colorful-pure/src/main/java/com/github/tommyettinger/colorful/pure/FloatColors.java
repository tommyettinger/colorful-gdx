...
public class FloatColors {
    ...
    public static float hsl2rgb(final float hsla) {
        final int decoded = BitConversion.floatToRawIntBits(hsla);
    ...
    public static float hsl2rgb(final float h, final float s, final float l, final float a){
        ...
        return BitConversion.intBitsToFloat(
                (int)(v * MathTools.lerp(1f, x, d)) |
                (int)(v * MathTools.lerp(1f, y, d)) << 8 |
                (int)(v * MathTools.lerp(1f, z, d)) << 16 |
                ((int)(a * 255.999f) << 24 & 0xFE000000));
    }
    ...
    public static float rgb2hsl(final float rgba) {
        final int decoded = BitConversion.floatToRawIntBits(rgba);
    ...
    public static float rgb2hsl(final float r, final float g, final float b, final float a) {
        ...
        return BitConversion.intBitsToFloat(
                (int)(255.999f * Math.abs(z + (w - y) / (6f * d + 1e-10f))) |
                        (int)(255.999f * (x - l) / (Math.min(l, 1f - l) + 1e-10f)) << 8 |
                        (int)(255.999f * l) << 16 |
                        ((int)(255.999f * a) << 24 & 0xFE000000));
    }
     ...
     *                     or {@link com.github.tommyettinger.colorful.pure.ipt.ColorTools#ipt(float, float, float, float)}
    ...
    public static float lerpFloatColors(final float start, final float end, float change) {
        ...
        return BitConversion.intBitsToFloat(((int) (ys + change * (ye - ys)) & 0xFF)
