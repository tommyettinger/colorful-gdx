...
public class ColorTools {
	...
	public static int toRGBA8888(final float packed)
	{
		...
		return MathTools.clamp(y + (cw * 5 >> 3) - cm, 0, 0xFF) << 24
				| MathTools.clamp(y - (cw * 3 >> 3) + cm, 0, 0xFF) << 16
				| MathTools.clamp(y - (cw * 3 >> 3) - cm, 0, 0xFF) << 8
	...
	public static float toRGBA(final float packed)
	{
		final int decoded = BitConversion.floatToRawIntBits(packed), y = (decoded & 0xff),
				cw = ((decoded >>> 7 & 0x1fe) - 0xfe),
				cm = (((decoded >>> 15 & 0x1fe) - 0xfe) >> 1);
		return BitConversion.intBitsToFloat(MathTools.clamp(y + (cw * 5 >> 3) - cm, 0, 0xFF)
				| MathTools.clamp(y - (cw * 3 >> 3) + cm, 0, 0xFF) << 8
				| MathTools.clamp(y - (cw * 3 >> 3) - cm, 0, 0xFF) << 16
				| (decoded & 0xfe000000));
	}
	...
	{
		final int decoded = BitConversion.floatToRawIntBits(encoded);
		return MathTools.clamp((decoded & 0xff) * 0x1.010102p-8f + ((decoded >>> 8 & 0xff) - 127.5f) * (0x1.414142p-9f) - ((decoded >>> 16 & 0xff) - 127.5f) * 0x1.010102p-9f, 0f, 1f);
	}
	...
	{
		final int decoded = BitConversion.floatToRawIntBits(encoded);
		return MathTools.clamp((decoded & 0xff) * 0x1.010102p-8f - (((decoded >>> 8 & 0xff) - 127.5f) * 0x1.818184p-10f) + ((decoded >>> 16 & 0xff) - 127.5f) * 0x1.010102p-9f, 0f, 1f);
	}
	...
	public static float saturation(final float encoded) {
		final int decoded = BitConversion.floatToRawIntBits(encoded), lu = (decoded & 0xff),
				cw = ((decoded >>> 7 & 0x1fe) - 0xfe),
				cm = (((decoded >>> 15 & 0x1fe) - 0xfe) >> 1);
		final float r = MathTools.clamp(lu + (cw * 5 >> 3) - cm, 0, 0xFF) * 0x1.010102p-8f;
		final float g = MathTools.clamp(lu - (cw * 3 >> 3) + cm, 0, 0xFF) * 0x1.010102p-8f;
		final float b =  MathTools.clamp(lu - (cw * 3 >> 3) - cm, 0, 0xFF) * 0x1.010102p-8f;
		float x, y, w;
		if(g < b) {
			x = b;
			y = g;
...
//        return (bits & 0xFF) * 0x3p-11f + (bits >>> 8 & 0xFF) * 0x1p-9f + (bits >>> 16 & 0xFF) * 0x1p-11f;
	...
	public static float toEditedFloat(float basis, float hue, float saturation, float value, float opacity) {
		...
		value = MathTools.clamp(value + (e & 0xff) * 0x1.010102p-8f, 0f, 1f);
		...
		return fromRGBA(FloatColors.hsl2rgb(hue2 + hue + 1 - (int)(hue2 + hue + 1), MathTools.clamp(saturation + sat2, 0f, 1f), value, opacity));
	}
	...
	public static float randomEdit(final float color, long seed, final float variance) {
		...
		return BitConversion.intBitsToFloat((decoded & 0xFE000000) | ((int)(MathTools.clamp(cm + c, -1, 1) * 127.5f + 128f) << 16 & 0xFF0000)
				| ((int)(MathTools.clamp(cw + b, -1, 1) * 127.5f + 128f) << 8 & 0xFF00) | (int)(MathTools.clamp(y + a, 0, 1) * 255f));
	}
	...
	{
		float y2 = y = MathTools.clamp(y, 0f, 1f);
		float cw2 = cw = MathTools.clamp((cw - 0.5f) * 2f, -1f, 1f);
		float cm2 = cm = MathTools.clamp((cm - 0.5f) * 2f, -1f, 1f);
		for (int attempt = 31; attempt >= 0; attempt--) {
			final float r = y2 + 0.625f * cw2 - cm2;
			final float g = y2 - 0.375f * cw2 + cm2;
			final float b = y2 - 0.375f * cw2 - cm2;
		...
		return ycwcm(y2, cw2 * 0.5f + 0.5f, cm2 * 0.5f + 0.5f, MathTools.clamp(a, 0f, 1f));
	}
