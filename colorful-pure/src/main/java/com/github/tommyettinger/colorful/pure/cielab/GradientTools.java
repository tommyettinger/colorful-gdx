 ...
 * The intent is for the FloatList to be used as a sequence of packed float CIELAB colors. You can create a new
...
public class GradientTools {
     ...
     * This limits individual steps of color to the correct CIELAB gamut, so even interpolations between colors at
     * extreme points in the color space will stay in-gamut.
     * @param start the packed float CIELAB color to start with
     * @param end the packed float CIELAB color to end on
     ...
     * individual steps of color to the correct CIELAB gamut, so even interpolations between colors at extreme points in
    ...
    /**
     * Appends a gradient between several packed float CIELAB colors provided in {@code chain}. This uses linear
     * Interpolation for the whole gradient. Appends to the end of {@code appending} and produces a total of
     * {@code steps} colors.
     * @param appending a FloatList that will be appended to
     * @param steps how many steps the gradient should use; usually greater than 2
     * @param chain an array or varargs of packed float CIELAB colors that this will interpolate through in order
     * @return {@code appending}, after adding the gradient to the end
     */
     ...
     * @param appending a FloatList that will be appended to
     * @param steps how many steps the gradient should use; usually greater than 2
     * @param chain a FloatList of packed float CIELAB colors that this will interpolate through in order
     * @return {@code appending}, after adding the gradient to the end
     */
    ...
    /**
     * Appends a gradient between several packed float CIELAB colors provided in {@code chain}. This uses the specified
    ...
    public static FloatList appendGradientChain(FloatList appending, int steps, Interpolation interpolation, FloatList chain) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            int idx = (int)splint;
            appending.add(ColorTools.limitToGamut(FloatColors.lerpFloatColors(chain.get(idx), chain.get(idx+1), MathTools.norm(idx * splits, idx * splits + splits, splint))));
            change += step;
        }
    ...
    public static FloatList appendGradientChain(FloatList appending, int steps, Interpolation interpolation, float... chain) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            float splint = Math.min(Math.max(interp * splits, 0f), splits - 0.000001f);
            int idx = (int)splint;
            appending.add(ColorTools.limitToGamut(FloatColors.lerpFloatColors(chain[idx], chain[idx+1], MathTools.norm(idx * splits, idx * splits + splits, splint))));
            change += step;
        }
     ...
     * @param appending a FloatList that will be appended to
     * @param start the packed float CIELAB color to start with
     * @param end the packed float CIELAB color to end just before
     * @param steps how many steps the gradient should use; usually greater than 2
