...
import static com.github.tommyettinger.colorful.pure.rgb.ColorTools.*;
 ...
 * {@code NAMED.get("cactus", 0f)}, or by index in the FloatList called {@link #LIST}. Note that to access a float
 * color from NAMED, you need to give a default value if the name is not found; {@code 0f} is a good default because it
 * is used for fully-transparent. You can access the names in a specific order with {@link #NAMES} (which is
...
public class SimplePalette {
    ...
    static {
        ...
        NAMES_BY_LIGHTNESS.sort((o1, o2) -> Float.compare(lightness(NAMED.get(o1)), lightness(NAMED.get(o2))));
    }
    ...
    public static String bestMatch(final float rgb, int mixCount) {
        ...
        float oklab = com.github.tommyettinger.colorful.pure.oklab.ColorTools.fromRGBA(rgb);
        float bestDistance = Float.POSITIVE_INFINITY;
        final int paletteSize = namesByHue.size(), colorTries = (int)Math.pow(paletteSize, mixCount), totalTries = colorTries * 81;
        final float targetL = com.github.tommyettinger.colorful.pure.oklab.ColorTools.channelL(oklab), targetA = com.github.tommyettinger.colorful.pure.oklab.ColorTools.channelA(oklab), targetB = com.github.tommyettinger.colorful.pure.oklab.ColorTools.channelB(oklab);
