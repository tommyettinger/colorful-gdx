...
public class ColorTools {
	...
	public static float fromRGBA8888(final int rgba) {
		...
		return BitConversion.intBitsToFloat(
						...
						| MathTools.clamp((int)((0.9889992500f * l - 1.2142961000f * m + 0.2252968500f * s + 0.5f) * 255.999f), 0, 255) << 8
						| MathTools.clamp((int)((0.0129520185f * l + 0.3913858800f * m - 0.4043378800f * s + 0.5f) * 255.999f), 0, 255) << 16
						| (rgba & 0xFE) << 24);
	}
...
//		return true;
	...
	public static float limitToGamut(final float packed) {
		...
		for (int attempt = 31; attempt >= 0; attempt--) {
			...
			L2 = MathTools.lerp(0.63f, L, progress);
			A2 = MathTools.lerp(0, A, progress);
			B2 = MathTools.lerp(0, B, progress);
		}
