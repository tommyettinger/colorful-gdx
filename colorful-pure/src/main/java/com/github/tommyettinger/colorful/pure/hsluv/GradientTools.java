 ...
 * The intent is for the FloatArray to be used as a sequence of packed float HSLuv colors. You can create a new
 * FloatArray gradient with {@link #makeGradient(float, float, int, InterpolationFunction)}, but any FloatArray will work
 ...
 * <br>
 * This class does some special handling for HSLuv colors.
 */
public class GradientTools {
    ...
    /**
     * Creates a FloatList gradient from the packed float HSLuv color {@code start} to the packed float HSLuv color
     * {@code end}, taking the specified number of steps and using linear interpolation.
     * @param start the packed float HSLuv color to start with
     * @param end the packed float HSLuv color to end on
    ...
    /**
     * Appends a gradient from the packed float HSLuv color {@code start} to the packed float HSLuv color {@code end},
    ...
    /**
     * Appends a gradient between several packed float HSLuv colors provided in {@code chain}. This uses linear
     * InterpolationFunction for the whole gradient. Appends to the end of {@code appending} and produces a total of
     * {@code steps} colors.
     * @param appending a FloatList that will be appended to
     * @param steps how many steps the gradient should use; usually greater than 2
     * @param chain an array or varargs of packed float HSLuv colors that this will interpolate through in order
     * @return {@code appending}, after adding the gradient to the end
     */
     ...
     * @param appending a FloatList that will be appended to
     * @param steps how many steps the gradient should use; usually greater than 2
     * @param chain a FloatList of packed float HSLuv colors that this will interpolate through in order
     * @return {@code appending}, after adding the gradient to the end
     */
    ...
    /**
     * Appends a gradient between several packed float HSLuv colors provided in {@code chain}. This uses the specified
    ...
    public static FloatList appendGradientChain(FloatList appending, int steps, InterpolationFunction interpolation, FloatList chain) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            int idx = (int)splint;
            appending.add(ColorTools.lerpFloatColors(chain.get(idx), chain.get(idx+1), MathTools.norm(idx, idx +1, splint)));
            change += step;
        }
    ...
    public static FloatList appendGradientChain(FloatList appending, int steps, InterpolationFunction interpolation, float... chain) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            float splint = Math.min(Math.max(interp * splits, 0f), splits - 0.000001f);
            int idx = (int)splint;
            appending.add(ColorTools.lerpFloatColors(chain[idx], chain[idx+1], MathTools.norm(idx, idx +1, splint)));
            change += step;
        }
     ...
     * @param appending a FloatList that will be appended to
     * @param start the packed float HSLuv color to start with
     * @param end the packed float HSLuv color to end just before
     * @param steps how many steps the gradient should use; usually greater than 2
    ...
    public static FloatList appendPartialGradient(FloatList appending, float start, float end, int steps, InterpolationFunction interpolation){
        ...
        float step = 1f / steps, change = 0f;
        for (int i = 0; i < limit; i++) {
            appending.add(ColorTools.lerpFloatColors(start, end, interpolation.apply(change)));
            change += step;
        }
        return appending;
    }
