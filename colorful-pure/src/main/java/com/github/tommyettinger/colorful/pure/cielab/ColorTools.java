...
public class ColorTools {
     ...
     * alpha (or opacity). L should be between 0 and 1, inclusive, with 0 used for very dark colors (almost only black),
     * and 1 used for very light colors (almost only white). A and B range from 0.0 to 1.0, with grayscale results when
     * both are about 0.5. There's some aesthetic value in changing just one chroma value. When A is high and B is low,
     * the color is more purple/magenta, when both are low it is more bluish, when B is high and A is low, the color
     * tends to be greenish, and when both are high it tends to be orange. When A and B are both near 0.5f, the color is
     * closer to gray. Alpha is the multiplicative opacity of the color, and acts like RGBA's alpha.
     *
    ...
    public static float cielab(float l, float a, float b, float alpha) {
        return BitConversion.intBitsToFloat(((int) (alpha * 255.999f) << 24 & 0xFE000000) | ((int) (b * 255.999f) << 16 & 0xFF0000)
    ...
    public static float chromaLimit(final float hue, final float lightness) {
        ...
        final float B = TrigTools.sinTurns(h) * (1.26365817f);
        final float y = reverseXYZ(L);
        float A2 = A, B2 = B;
    ...
    public static float cielabByHSL(float hue, float saturation, float lightness, float alpha) {
        ...
        hue -= MathTools.floor(hue);
        alpha = Math.min(Math.max(alpha, 0f), 1f);
        final float L = (1f/1.16f)*(lightness + 0.16f);
        final float L0 = (1f/1.16f)*(L + 0.16f);
        final float cos = TrigTools.cosTurns(hue);
        final float sin = TrigTools.sinTurns(hue);
    ...
    public static float cielabByHCL(float hue, float chroma, float lightness, float alpha) {
        ...
        return BitConversion.intBitsToFloat(
                (int) (alpha * 127.999f) << 25 |
                        Math.min(Math.max((int) (TrigTools.sinTurns(hue) /* * 2f */ * chroma + 127.5f), 0), 255) << 16 |
     ...
     * different than the result of {@code basis} unless hue, saturation, lightness, and opacity are all 0.
    ...
    public static float differentiateLightness(final float mainColor, final float contrastingColor)
    {
        ...
        return BitConversion.intBitsToFloat((main & 0xFEFFFF00) | (contrast + 128 & 0xFF) + (main & 0xFF) >>> 1);
    }
    ...
    public static float offsetLightness(final float mainColor) {
        ...
        return BitConversion.intBitsToFloat((decoded & 0xFEFFFF00) | (decoded + 128 & 0xFF) + (decoded & 0xFF) >>> 1);
    }
     ...
     * 50% gray, and when it is in-between 0.0 and 1.0 it returns something between the two. This is
