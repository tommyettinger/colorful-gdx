...
public class HsluvTintDemo extends ApplicationAdapter {
...
//    public static final int SCREEN_HEIGHT = 862;
    ...
    private float H = 0f, S = 0.5f, L = 0.5f, opacity = 1f;
...
//        config.setResizable(false);
        ...
        final HsluvTintDemo app = new HsluvTintDemo();
    ...
    public void create() {
        ...
        shader = new ShaderProgram(Shaders.vertexShaderHsluv, Shaders.fragmentShaderHsluv);
...
//        load("C:/d/Art/translucent-bubble.png");
    ...
    public void render() {
        ...
        if (screenTexture != null) {
            ...
            batch.setColor(H -= MathUtils.floor(H), S =MathUtils.clamp(S, 0f, 1f), L =MathUtils.clamp(L, 0f, 1f), opacity);
            batch.begin();
...
//                if (ColorTools.inGamut(L - 0x3p-7f, A, B))
                    L -= 0x1p-7f;
            } else if (input.isKeyPressed(Input.Keys.RIGHT)) //raise H
            {
//                if (ColorTools.inGamut(L, A + 0x3p-7f, B))
                    H += 0x1p-7f;
            } else if (input.isKeyPressed(Input.Keys.LEFT)) //lower H
            {
//                if (ColorTools.inGamut(L, A - 0x3p-7f, B))
                    H -= 0x1p-7f;
            } else if (input.isKeyPressed(Input.Keys.UP)) //raise S
            {
//                if (ColorTools.inGamut(L, A, B + 0x3p-7f))
                    S += 0x1p-7f;
            } else if (input.isKeyPressed(Input.Keys.DOWN)) //lower S
            {
//                if (ColorTools.inGamut(L, A, B - 0x3p-7f))
                    S -= 0x1p-7f;
