...
//        for(OrderedMap.Entry<String, Float> entry : viewer){
    ...
    private float difference(float left, float right) {
        final float i = (ColorTools.intensity(left) - ColorTools.intensity(right)) * 2f;
        final float p = ColorTools.protan(left) - ColorTools.protan(right);
        final float t = ColorTools.tritan(left) - ColorTools.tritan(right);
