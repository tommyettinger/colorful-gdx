...
public class HardReplaceDemo extends ApplicationAdapter {
    public static final int SCREEN_WIDTH = 512;
    public static final int SCREEN_HEIGHT = 224;
    protected SpriteBatch batch;
    protected SpriteBatch colorizeBatch;
    ...
    public static void main(String[] arg) {
        ...
        config.setTitle("Swap White Demo");
        config.setWindowedMode(SCREEN_WIDTH, SCREEN_HEIGHT);
        config.setIdleFPS(10);
        config.useVsync(true);
//        config.setResizable(false);

        final HardReplaceDemo app = new HardReplaceDemo();
    ...
    public void create() {
        ...
        ShaderProgram shader = new ShaderProgram(Shaders.vertexShader, Shaders.fragmentShaderInvertedRGB);
        if(!shader.isCompiled()){
            System.out.println("Shader failed to compile: " + shader.getLog());
            shader = null;
        }
        colorizeBatch = Shaders.makeSwappingBatch(1f, 0f, 0f);
...
//        load("samples/Among_the_Sierra_Nevada_by_Albert_Bierstadt.jpg");
    ...
    public void handleInput() {
        ...
        else if (input.isKeyPressed(Input.Keys.A)) // abstract gradient
            load("samples/gradients.png");
