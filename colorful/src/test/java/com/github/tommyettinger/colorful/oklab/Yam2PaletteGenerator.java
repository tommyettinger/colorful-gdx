...
public class Yam2PaletteGenerator extends ApplicationAdapter {
    ...
    public static void main(String[] arg) {
        ...
        new Lwjgl3Application(new Yam2PaletteGenerator(), config);
    }
    /**
    ...
    public void create(){
        ...
        for (int wave = 1; wave <= 4; wave++) {
            ...
            for (int i = 0; i < hueKeys.length; i++) {
                float hue = hueKeys[i], quart = wave * 0.25f, fraction = i / (float)hueKeys.length,
                        ...
                        satAdjust = (fraction >= (0.06f) && fraction < (0.13f) ? 0.75f - TrigTools.sin_((fraction - 0.06f) * 20f) * 0.25f : 1.0f) * (1f - (i & 1) * 0.1f);
                ...
                for (int j = 0, cr = 1; j < crest; j++, cr += 2) {
                    if(crest == 1)
                        pal.add(oklabByHCL(hue, lerp(0.08f, outerC, quart) * satAdjust, outerL, 1f));
                    else
                        pal.add(oklabByHCL(hue, lerp(0.025f, outerC, quart) * satAdjust, lerp(minL, maxL, cr / (crest * 2f)), 1f));
        ...
        Gdx.files.local("Yam2ColorData.txt").writeString(sb2.toString(), false, "UTF8");
