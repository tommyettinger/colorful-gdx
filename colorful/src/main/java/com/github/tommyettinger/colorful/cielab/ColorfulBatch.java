 ...
 * {@link Palette} or generated with {@link ColorTools}). This ColorfulBatch uses the CIELAB color space where
...
public class ColorfulBatch implements Batch {
     ...
     */
    public static final float TWEAK_RESET = ColorTools.cielab(0.5f, 0.5f, 0.5f, 0.5f);
    ...
    /**
     * Sets the color to the result of {@link ColorTools#cielab(float, float, float, float)} on the same arguments.
     ...
     * @see ColorTools#cielab(float, float, float, float)
    ...
    /**
     * Expects an int color in the format (luma, 8 bits), (chroma A, 8 bits), (chroma B, 8 bits), (alpha, 7 bits),
     * (ignored, 1 bit).
     * @param color an int color with alpha in the least significant byte and luma in the most significant
     */
