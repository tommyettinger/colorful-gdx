...
import static com.github.tommyettinger.colorful.ipt_hq.ColorfulBatch.*;
...
public class ColorfulSprite extends TextureRegion {
	 ...
	 * Use {@link ColorTools#ipt(float, float, float, float)} or a predefined color from {@link Palette} if you
	 * don't have a color currently.
	 * @param color the packed float color used to add intensity, protan, and tritan to the current sprite, as well as the multiplier for alpha
	 */
	 ...
	 * get a tweak value with {@link ColorTools#ipt(float, float, float, float)}, just using the last parameter
	 * to represent contrast.
	 * @param color the packed float color used to add intensity, protan, and tritan to the current sprite, as well as the multiplier for alpha
	 * @param tweak the packed float used to multiply intensity, protan, and tritan, as well as the setting for contrast   
	 */
	 ...
	 * get a tweak value with {@link ColorTools#ipt(float, float, float, float)}, just using the last parameter
	 * to represent contrast.
	 * @param intensityAdd how much lightness/intensity to add; darkest is 0f, neutral is 0.5f, lightest is 1f
	 * @param protanAdd how much to add on the cyan/red axis; more cyan is 0f, neutral is 0.5f, more red is 1f
	 * @param tritanAdd how much to add on the blue/yellow axis; more blue is 0f, neutral is 0.5f, more yellow is 1f
	 * @param alphaMul how much to multiply alpha by; fully transparent is 0f, neutral is 1f
	 * @param intensityMul how much source lightness/intensity should be multiplied by; darkest is 0f, neutral is 0.5f, lightest is 1f
	 * @param protanMul how much the source's cyan/red axis should be multiplied by; no cyan/red is 0f, neutral is 0.5f, max cyan/red is 1f
	 * @param tritanMul how much the source's blue/yellow axis should be multiplied by; no blue/yellow is 0f, neutral is 0.5f, max blue/yellow is 1f
	 ...
	 */
	public void setTweakedColor (float intensityAdd, float protanAdd, float tritanAdd, float alphaMul, float intensityMul, float protanMul, float tritanMul, float contrast) {
		final float color = ColorTools.ipt(intensityAdd, protanAdd, tritanAdd, alphaMul),
				tweak = ColorTools.ipt(intensityMul, protanMul, tritanMul, contrast); 
		float[] vertices = this.vertices;
		vertices[C1] = color;
		vertices[C2] = color;
		vertices[C3] = color;
	 ...
	 * state. You can easily get a tweak value with {@link ColorTools#ipt(float, float, float, float)}, just
	...
	/**
	 * Given an RGBA8888 Color from libGDX, this converts that to an IPT color as a packed float and sets the color of
	...
	public void setColor (float intensity, float protan, float tritan, float alpha) {
		final float color = ColorTools.ipt(intensity, protan, tritan, alpha);
		final float[] vertices = this.vertices;
		vertices[C1] = color;
		vertices[C2] = color;
		vertices[C3] = color;
	...
	public void setTweak (float intensity, float protan, float tritan, float contrast) {
		final float tweak = ColorTools.ipt(intensity, protan, tritan, contrast);
		final float[] vertices = this.vertices;
		vertices[C1] = tweak;
		vertices[C2] = tweak;
		vertices[C3] = tweak;
	 ...
	 * afterward.
	 * @return a packed float color used to add intensity, protan, and tritan to the current sprite, as well as the multiplier for alpha
	 */
	 ...
	 * @return a packed float used to multiply intensity, protan, and tritan, as well as the setting for contrast   
	 */
