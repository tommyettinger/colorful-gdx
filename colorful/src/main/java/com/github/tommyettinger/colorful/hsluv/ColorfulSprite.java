...
import static com.github.tommyettinger.colorful.hsluv.ColorfulBatch.*;

/**
 * Holds the geometry, color, and texture information for drawing 2D sprites using {@link ColorfulBatch}. A
 * ColorfulSprite has a position and a size given as width and height. The position is relative to the origin of the
 * coordinate system specified via {@link ColorfulBatch#begin()} and the respective matrices. A ColorfulSprite is always
 * rectangular and its position (x, y) are located in the bottom left corner of that rectangle. A ColorfulSprite also
 * has an origin around which rotations and scaling are performed (that is, the origin is not modified by rotation and
 * scaling). The origin is given relative to the bottom left corner of the ColorfulSprite, its position.
 * @author mzechner
 * @author Nathan Sweet 
...
public class ColorfulSprite extends TextureRegion {
	 ...
	 * Use {@link ColorTools#hsluv(float, float, float, float)} or a predefined color from {@link Palette} if you
	 * don't have a color currently.
	 * @param color the packed float color used to add to the H/S/L channels of the current sprite, as well as the multiplier for alpha
	 */
	 ...
	 * get a tweak value with {@link ColorTools#hsluv(float, float, float, float)}, just using the last parameter
	 * to represent contrast.
	 * @param color the packed float color used to add to the H/S/L channels of the current sprite, as well as the multiplier for alpha
	 * @param tweak the packed float used to multiply the H/S/L channels, as well as the setting for contrast
	 */
	...
	public void setTweakedColor (float addH, float addS, float addL, float mulAlpha, float mulH, float mulS, float mulL, float contrast) {
		final float color = ColorTools.hsluv(addH, addS, addL, mulAlpha),
				tweak = ColorTools.hsluv(mulH, mulS, mulL, contrast);
		float[] vertices = this.vertices;
		vertices[C1] = color;
		vertices[C2] = color;
		vertices[C3] = color;
	 ...
	 * state. You can easily get a tweak value with {@link ColorTools#hsluv(float, float, float, float)}, just
	...
	/**
	 * Given an RGBA8888 Color from libGDX, this converts that to an HSLuv color as a packed float and sets the color of
	...
	public void setColor (float h, float s, float l, float alpha) {
		final float color = ColorTools.hsluv(h, s, l, alpha);
		final float[] vertices = this.vertices;
		vertices[C1] = color;
		vertices[C2] = color;
		vertices[C3] = color;
	...
	public void setTweak (float h, float s, float l, float contrast) {
		final float tweak = ColorTools.hsluv(h, s, l, contrast);
		final float[] vertices = this.vertices;
		vertices[C1] = tweak;
		vertices[C2] = tweak;
		vertices[C3] = tweak;
	 ...
	 * afterward.
	 * @return a packed float color used to add to the H/S/L channels of the current sprite, as well as the multiplier for alpha
	 */
	 ...
	 * @return a packed float used to multiply the H/S/L channels, as well as the setting for contrast
	 */
