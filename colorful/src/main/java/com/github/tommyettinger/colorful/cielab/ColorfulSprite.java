...
import static com.github.tommyettinger.colorful.cielab.ColorfulBatch.*;
...
public class ColorfulSprite extends TextureRegion {
	 ...
	 * Use {@link ColorTools#cielab(float, float, float, float)} or a predefined color from {@link Palette} if you
	 ...
	 * get a tweak value with {@link ColorTools#cielab(float, float, float, float)}, just using the last parameter
	...
	public void setTweakedColor (float addL, float addA, float addB, float mulAlpha, float mulL, float mulA, float mulB, float contrast) {
		final float color = ColorTools.cielab(addL, addA, addB, mulAlpha),
				tweak = ColorTools.cielab(mulL, mulA, mulB, contrast);
		float[] vertices = this.vertices;
		vertices[C1] = color;
		vertices[C2] = color;
		vertices[C3] = color;
	 ...
	 * state. You can easily get a tweak value with {@link ColorTools#cielab(float, float, float, float)}, just
	...
	public void setColor (float L, float A, float B, float alpha) {
		final float color = ColorTools.cielab(L, A, B, alpha);
		final float[] vertices = this.vertices;
		vertices[C1] = color;
		vertices[C2] = color;
		vertices[C3] = color;
	...
	public void setTweak (float L, float A, float B, float contrast) {
		final float tweak = ColorTools.cielab(L, A, B, contrast);
		final float[] vertices = this.vertices;
		vertices[C1] = tweak;
		vertices[C2] = tweak;
		vertices[C3] = tweak;
