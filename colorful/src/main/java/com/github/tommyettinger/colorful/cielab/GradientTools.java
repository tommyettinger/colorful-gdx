 ...
 * The intent is for the FloatArray to be used as a sequence of packed float CIELAB colors. You can create a new
...
public class GradientTools {
    ...
    /**
     * Creates a FloatArray gradient from the packed float CIELAB color {@code start} to the packed float CIELAB color
     ...
     * @param appending a FloatArray that will be appended to
     * @param steps how many steps the gradient should use; usually greater than 2
     * @param chain a FloatArray of packed float CIELAB colors that this will interpolate through in order
     * @return {@code appending}, after adding the gradient to the end
     */
    ...
    public static FloatArray appendGradientChain(FloatArray appending, int steps, Interpolation interpolation, FloatArray chain) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            int idx = (int)splint;
            appending.add(ColorTools.limitToGamut(FloatColors.lerpFloatColors(chain.get(idx), chain.get(idx+1), MathUtils.norm(idx * splits, idx * splits + splits, splint))));
            change += step;
        }
