...
//		return (x - li); // (Math.min(li, 1f - li) + 1e-10f);
	 ...
	 * result of {@link #rgb(float, float, float, float)} unless hue, saturation, lightness, and opacity are all 0. This
	 * won't allocate any objects.
	 * <br>
	...
	public static float toEditedFloat(float basis, float hue, float saturation, float light, float opacity) {
		...
		final float lum = Math.min(Math.max(x * (1f - 0.5f * d / (x + 1e-10f)) + light, 0f), 1f);
