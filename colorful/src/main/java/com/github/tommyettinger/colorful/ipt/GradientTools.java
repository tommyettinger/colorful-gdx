 ...
 * The intent is for the FloatArray to be used as a sequence of packed float IPT colors. You can create a new
...
public class GradientTools {
    ...
    /**
     * Creates a FloatArray gradient from the packed float IPT color {@code start} to the packed float IPT color
     * {@code end}, taking the specified number of steps and using linear interpolation.
     * @param start the packed float IPT color to start with
     * @param end the packed float IPT color to end on
    ...
    /**
     * Appends a gradient from the packed float IPT color {@code start} to the packed float IPT color {@code end},
    ...
    /**
     * Appends a gradient between several packed float IPT colors provided in {@code chain}. This uses linear
     * Interpolation for the whole gradient. Appends to the end of {@code appending} and produces a total of
     * {@code steps} colors.
     * @param appending a FloatArray that will be appended to
     * @param steps how many steps the gradient should use; usually greater than 2
     * @param chain an array or varargs of packed float IPT colors that this will interpolate through in order
     * @return {@code appending}, after adding the gradient to the end
     */
     ...
     * @param appending a FloatArray that will be appended to
     * @param steps how many steps the gradient should use; usually greater than 2
     * @param chain a FloatArray of packed float IPT colors that this will interpolate through in order
     * @return {@code appending}, after adding the gradient to the end
     */
    ...
    /**
     * Appends a gradient between several packed float IPT colors provided in {@code chain}. This uses the specified
    ...
    public static FloatArray appendGradientChain(FloatArray appending, int steps, Interpolation interpolation, FloatArray chain) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            int idx = (int)splint;
            appending.add(FloatColors.lerpFloatColors(chain.get(idx), chain.get(idx+1), MathUtils.norm(idx * splits, idx * splits + splits, splint)));
            change += step;
        }
    ...
    public static FloatArray appendGradientChain(FloatArray appending, int steps, Interpolation interpolation, float... chain) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            float splint = Math.min(Math.max(interp * splits, 0f), splits - 0.000001f);
            int idx = (int)splint;
            appending.add(FloatColors.lerpFloatColors(chain[idx], chain[idx+1], MathUtils.norm(idx * splits, idx * splits + splits, splint)));
            change += step;
        }
     ...
     * @param appending a FloatArray that will be appended to
     * @param start the packed float IPT color to start with
     * @param end the packed float IPT color to end just before
     * @param steps how many steps the gradient should use; usually greater than 2
